...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        part1 = -scores[np.arange(N), y]
        temp = np.sum(np.exp(scores), axis = 1)
        part2 = np.log(temp)
        part_reg = 0.5*reg*np.sum(W1**2) + 0.5*reg*np.sum(W2**2)
        loss = np.sum(part1 + part2) / N + part_reg
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        # of same size                                                         #
        ########################################################################
        d_scores = np.exp(scores) / temp.reshape((N, 1))
        d_scores[np.arange(N), y] -= 1
        grads['b2'] = np.sum(d_scores, axis=0) / N
        grads['W2'] = np.dot(A1.T, d_scores) / N + reg*W2
        dA1 = np.dot(d_scores, W2.T)
        ReLU_grads = (Z1 > 0) * 1
        dZ1 = np.multiply(dA1, ReLU_grads)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    hidden_size = 150
    input_size = 32 * 32 * 3
    net_ = TwoLayerNet(input_size, hidden_size, 10)
    learning_rate = [1e-1, 5e-1, 1e-2, 5e-2]
    regularization_strengths = [0.25, 0.5, 1, 1.5, 2]
    stats = net_.train(X_train, y_train, X_val, y_val,
            num_iters=2000, batch_size=500,
            learning_rate=1e-3, learning_rate_decay=0.95,
            reg=0.05, verbose=True)
    
    val_acc = (net_.predict(X_val) == y_val).mean()
